...
test("Parse What's Cooking mail", async () => {
    ...
    expect(sousChef.messageID).toEqual("xmqqim6zz8x6.fsf@gitster.c.googlers.com");
    const branchInfo = sousChef.branches.get("js/rebase-i-commit-cleanup-fix");
    expect(branchInfo).not.toBeUndefined();
    expect(branchInfo?.merged).toEqual("(merged to 'next' on 2021-01-31 at 358f562e1f)");
    expect(branchInfo?.sectionName).toEqual("Graduated to 'master'");
    const text = `When "git rebase -i" processes "fixup" insn, there is no reason to
