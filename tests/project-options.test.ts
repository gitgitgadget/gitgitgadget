...
import { isDirectory, testCommit, testCreateRepo } from "./test-lib";
...
test("project options", async () => {
    ...
    expect(await testCommit(workDir, "initial")).toEqual(initialCommit);
    ...
    expect(await git(["checkout", "-b", "test-project-options"], { workDir }))
        .toEqual("");
    expect(await testCommit(workDir, "A")).not.toEqual("");
    expect(await testCommit(workDir, "B")).not.toEqual("");
    expect(await testCommit(workDir, "C")).not.toEqual("");
